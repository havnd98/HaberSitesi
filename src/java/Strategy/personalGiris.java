 ...
 * @author havnd
 */
public class personalGiris implements GirisStrategy {
    ...
    public String login(Giris g) {
        try {
            ...
            ResultSet rs = st.executeQuery("SELECT * FROM personal WHERE mail='" + g.getMail() + "' AND passwd='" + g.getPasswd() + "'");

            if (rs.next()) {
                setDogrulama("personal");
            } else {
                setDogrulama("");
            }
