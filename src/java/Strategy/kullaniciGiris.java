 ...
 * @author havnd
 */
public class kullaniciGiris implements GirisStrategy {
    ...
    public String login(Giris g) {
        try {
            ...
            ResultSet rs = st.executeQuery("SELECT * FROM musteri WHERE mail='" + g.getMail() + "' AND passwd='" + g.getPasswd() + "'");

            if (rs.next()) {
                setDogrulama("kullanici");
            } else {
                setDogrulama("");
            }
